...
class EjecutorAcciones:
    ...
    def ejecutar(self, accion: str, parametro):
        try:
            if accion == "abrir_app":
                ...
                return False, f"Accion no reconocida: {accion}"
    ...
    def _abrir_app(self, nombre_app: str):
        ...
        return True, f"Abriendo aplicacion: {nombre_app}"
    ...
    def _control_volumen(self, direccion: str):
        ...
        if codigo is None:
            return False, f"Direccion de volumen invalida: {direccion}"
    ...
    def _bloquear_sesion(self):
        if self.modo_prueba:
            return True, "[MODO PRUEBA] Se habria bloqueado la sesion (no ejecutado)"
        ctypes.windll.user32.LockWorkStation()
        return True, "Sesion bloqueada"

    def _apagar_equipo(self):
        if self.modo_prueba:
            return True, "[MODO PRUEBA] Se habria apagado el equipo (no ejecutado)"
    ...
    def _navegar(self, comando: str):
        try:
            ...
            return False, f"No se pudo cargar el modulo de navegacion. Detalle: {e}"
    ...
    def _listar_comandos(self):
        try:
            ...
            return False, f"No se pudo cargar el modulo de voz. Detalle: {e}"
        try:
            with open(self.ruta_config, "r", encoding="utf-8") as f:
                data = json.load(f)
            texto_ayuda = data.get("ayuda_hablada", "No hay ayuda configurada todavia.")
        except Exception as e:
