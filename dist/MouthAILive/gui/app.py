"""
Interfaz Grafica - Mouth AI Live

Diseno "manos libres": el microfono se abre UNA sola vez al arrancar y se
queda escuchando todo el tiempo. "Pausar" no lo apaga, solo hace que el
sistema ignore comandos hasta que se diga "reanuda".

Soporta correr tanto como script normal (python gui/app.py) como empacado
en un .exe con PyInstaller: la ubicacion de las carpetas del proyecto se
calcula distinto en cada caso (ver bloque RAIZ mas abajo).
...
# ---------------------------------------------------------------------------
# Calculo de la carpeta raiz del proyecto.
# - Si el programa corre como script normal (python gui/app.py), la raiz
#   es la carpeta que contiene a "gui".
# - Si el programa corre empacado como .exe (PyInstaller), sys.frozen es
#   True y la raiz es la carpeta donde quedo el .exe (ahi deben estar
#   copiadas a mano las carpetas asr/, interpreter/, actions/, etc.).
# ---------------------------------------------------------------------------
